...
- **Roue de la chance** (`#wheel-handle` → `#wheel-modal`) : 1 spin gagné toutes les
  ...
  (`computeWheelRewards`, une par segment, distinctes) : chaque slice = un disque planétaire
  en dégradé radial (froid bleu → brûlant rouge, `WHEEL_TIERS`) qui **affiche le #rang à
  gagner** → la rareté se lit d'un coup d'œil. Le noyau (`.wheel-hub`, `--hub`) montre
