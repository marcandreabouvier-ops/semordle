...
- **Roue de la chance** (`#wheel-handle` → `#wheel-modal`) : 1 spin gagné toutes les
  ...
  Les 12 récompenses sont **pré-tirées** (`computeWheelRewards`) : on choisit 12 mots distincts
  biaisés vers le proche, on les trie, et on donne le **plus proche au palier le plus rare**
  (`WHEEL_LAYOUT`) → le #rang est **toujours monotone avec la couleur** (rouge jackpot = plus
  petit, bleu modeste = plus grand), même quand le pool est mince (fin de partie). Chaque slice
  ...
  de slice vide → pas de spin gâché). Le gain est TOUJOURS un mot plus proche que bestRank
  (`eligibleWheelPool`). Le mot est ajouté via `applyWheelUnlock` (unlock marqué 🔓, compte dans
  unlockCount du partage, dot sur le radar). Jackpot/great = feux d'artifice.
