...
- **Roue de la chance** (`#wheel-handle` → `#wheel-modal`) : 1 spin gagné toutes les
  ...
  atterrit sur un segment tiré au hasard (fair). Les 12 récompenses sont **pré-tirées**
  (`computeWheelRewards`, une par segment, distinctes). Chaque slice a un fond **gris neutre**
  (deux nuances alternées) et porte une petite **planète ombrée** (dégradé sphère
  `light→color→rim` de `WHEEL_TIERS`, taille égale, halo sur great/jackpot) = le corps qui
  gravitera sur le radar une fois gagné, avec le **#rang à gagner** dessous → la rareté se
  lit d'un coup d'œil (bleu modeste → rouge jackpot). Le noyau (`.wheel-hub`, `--hub`) montre
  l'étoile équipée du joueur (`skinById(_profile.equipped)`). Le gain est TOUJOURS un mot
  plus proche que bestRank (`eligibleWheelPool` trié par bande de rareté modest→jackpot).
  Le mot est ajouté via `applyWheelUnlock` (unlock marqué 🔓, compte dans unlockCount du
  partage, dot sur le radar). Jackpot/great = feux d'artifice.
